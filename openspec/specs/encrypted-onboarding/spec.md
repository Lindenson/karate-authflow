# encrypted-onboarding Specification

## Purpose
TBD - created by archiving change encrypted-onboarding-strategy. Update Purpose after archive.
## Requirements
...
The strategy SHALL support the `standard` flavor (4 steps, built-in crypto backend public key + `pkr` from config) and the `handshake` flavor (5 steps; Step 0 `POST /api/v1/init/initial_handshake_key` is plain JSON and yields the per-device crypto backend public key used to wrap the RGK at Step 1).

#### Scenario: handshake handshake captured
- **WHEN** Step 0 returns `{ihshky}`
- **THEN** the strategy stores the crypto backend public key and `pkr` and uses them to wrap the RGK at Step 1

#### Scenario: standard skips the handshake
- **WHEN** the strategy is configured for the standard flavor
- **THEN** no Step 0 is required and the configured built-in crypto backend public key + `pkr` are used at Step 1
