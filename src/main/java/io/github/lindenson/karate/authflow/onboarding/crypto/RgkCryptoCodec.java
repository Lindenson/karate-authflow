...
/**
 * Plain-JCE crypto primitives for the crypto backend onboarding (RGK) flow, plus the wire
 * base64 "matryoshka" helpers.
 *
 * <p>All parameters are fixed by the server (verified against {@code the crypto backend}):
 * <ul>
 *   <li>RGK payloads: {@code AES/CBC/PKCS5Padding}, IV = 16 zero bytes, key = the raw RGK.</li>
 *   <li>RGK wrap: {@code RSA/ECB/PKCS1Padding} under the crypto backend public key (X509 DER).</li>
 * </ul>
 *
...
public final class RgkCryptoCodec {
    ...
    public static byte[] generateRgk(int bits) {
        try {
            ...
            throw new CryptoCodecException("Could not generate RGK", e);
        }
    }

    /**
     * Wrap the RGK under the crypto backend public key ({@code RSA/ECB/PKCS1Padding}).
     *
     * @param rgk                 raw RGK bytes
     * @param serverPublicKeyX509Der crypto backend public key, X509-encoded DER
     * @return the RSA-wrapped RGK bytes (binary, before any base64)
     */
    public static byte[] wrapRgk(byte[] rgk, byte[] serverPublicKeyX509Der) {
        try {
            PublicKey serverPub = KeyFactory.getInstance("RSA")
                    .generatePublic(new X509EncodedKeySpec(serverPublicKeyX509Der));
            Cipher rsa = Cipher.getInstance("RSA/ECB/PKCS1Padding");
            rsa.init(Cipher.ENCRYPT_MODE, serverPub);
            return rsa.doFinal(rgk);
        } catch (GeneralSecurityException e) {
            throw new CryptoCodecException("Could not wrap RGK under the crypto backend public key", e);
        }
    }
    ...
    private static byte[] aes(int mode, byte[] input, byte[] rgk) {
        try {
            ...
            throw new CryptoCodecException("RGK AES operation failed", e);
        }
    }
     ...
     *
     * @param rgk                 raw RGK key bytes
     * @param serverPublicKeyX509Der crypto backend public key, X509 DER
     * @return the inner {@code byte[]} to assign to the envelope's {@code rgke}
     */
    public static byte[] buildRgkeField(byte[] rgk, byte[] serverPublicKeyX509Der) {
        return innerB64Bytes(wrapRgk(rgk, serverPublicKeyX509Der));
    }
