...
/**
 * In-process model of the softpos-core TMS onboarding endpoints, doing the real server-side crypto
 * so tests are a trustworthy oracle (not a mirror of the strategy). It owns an RSA key pair whose
 * public key is the TMS public key; it unwraps {@code rgke}, decrypts {@code ed}, and encrypts
 * {@code ecd} exactly as the server does (decode order taken from {@code AbstractEncryptedRequestProcessor}
 * / {@code CryptographyService}).
 */
final class FakeTms implements AutoCloseable {
    ...
    private final KeyPair tms;
    private final Map<String, byte[]> rgkByRid = new ConcurrentHashMap<>();

    /** deviceSn token as the wire carries it: base64 of the SN bytes. */
    static final String DEVICE_SN_TOKEN = Base64.getEncoder().encodeToString("DEVSN-0001".getBytes(StandardCharsets.UTF_8));
    static final String MTMK = "MTMK-abc123";
    static final String MTTK = "MTTK-xyz789";

    FakeTms() throws Exception {
        KeyPairGenerator kpg = KeyPairGenerator.getInstance("RSA");
        kpg.initialize(2048);
        this.tms = kpg.generateKeyPair();
    ...
    /** The TMS public key (X509 DER) — what the standard flavor configures as built-in. */
    byte[] tmsPublicKeyX509() {
        return tms.getPublic().getEncoded();
    }
    ...
    private void handshake(HttpExchange ex) throws IOException {
        readBody(ex); // {dfrgprt}
        String ihshky = Base64.getEncoder().encodeToString(tmsPublicKeyX509());
        respond(ex, 200, "{\"ihshky\":\"" + ihshky + "\"}");
    }
    ...
    private void init(HttpExchange ex) throws IOException {
        try {
            ...
            decryptEd(env, rgk); // exercises the request payload decrypt
            String payload = "{\"deviceSn\":\"" + DEVICE_SN_TOKEN + "\"}";
    ...
    // ----- server-side crypto (decode exactly like softpos-core) -----

    /** A byte[] envelope field on the wire is base64 (Jackson) of the DTO byte[]; decode that layer. */
    private static byte[] wireFieldBytes(String wireValue) {
        return Base64.getDecoder().decode(wireValue);
    }

    /** rid/dsn/pkr: the DTO byte[] content is the UTF-8 of the string. */
    private static String dtoString(String wireValue) {
        return new String(wireFieldBytes(wireValue), StandardCharsets.UTF_8);
    }

    /** ed/rgke: DTO byte[] content is an inner base64 string; decode twice to binary. */
    private static byte[] innerBinary(String wireValue) {
        String innerB64 = new String(wireFieldBytes(wireValue), StandardCharsets.UTF_8);
    ...
    private byte[] rsaUnwrap(byte[] wrapped) throws Exception {
        ...
        c.init(Cipher.DECRYPT_MODE, (PrivateKey) tms.getPrivate());
