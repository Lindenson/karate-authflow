...
/**
 * In-process model of a crypto backend's onboarding endpoints, doing the real server-side crypto
 * so tests are a trustworthy oracle (not a mirror of the strategy). It owns an RSA key pair whose
 * public key is the backend public key; it unwraps {@code rgke}, decrypts {@code ed}, and encrypts
 * {@code ecd} exactly as a server would (the inverse of the strategy's request encoding).
 */
final class FakeCryptoBackend implements AutoCloseable {
    ...
    /** The crypto backend public key (X509 DER) — what the standard flavor configures as built-in. */
    ...
    private void handshake(HttpExchange ex) throws IOException {
        readBody(ex); // {dfrgprt}
        String ihshky = Base64.getEncoder().encodeToString(serverPublicKeyX509());
    ...
    // ----- server-side crypto (decode exactly like the crypto backend) -----
    ...
    private byte[] rsaUnwrap(byte[] wrapped) throws Exception {
        ...
        c.init(Cipher.DECRYPT_MODE, (PrivateKey) backend.getPrivate());
